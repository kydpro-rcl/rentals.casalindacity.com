	 ...
	 if(!$cant_edit>0){
				    ...
				    if ($_SESSION['info']['level']==1){
				   	   if ($qty_nights<1){
				             	...
				             	echo "<br/>";
				             	echo "<span style='color:red'Buyer rental must be 1 night</span>";
				       }else{
				...
				?>
				<h2 style="text-align:center; color:red;">Changing Buyer Rental <span style="color:black">No.<?=$_POST['reference']?></span></h2>
					 	    ...
					 	    <? }*/?>
					 	     <hr />
					<form method="post" action="edit_buyer3.php" name="long_2">
				...
				<fieldset><legend>Services for Long Term</legend>
					       	 ...
					       	 <?   // $casa['bed']=2;
					       	  switch($casa['bed']){
				                	...
				                	case 3: if($price_maid==60) $seleccionado='selected="selected"'; echo "<option value=\"60.00\" $seleccionado >A day per week (60.00)</option>";
											if($price_maid==80) $seleccionado='selected="selected"'; echo "<option value=\"80.00\" $seleccionado >Two days per week (80.00)</option>";
				                			break;
				                	case 4: if($price_maid==70) $seleccionado='selected="selected"'; echo "<option value=\"70.00\" $seleccionado >A day per week (70.00)</option>";
											if($price_maid==90) $seleccionado='selected="selected"'; echo "<option value=\"90.00\" $seleccionado >Two days per week (90.00)</option>";
				                			break;
				 ...
				 <p><span style="font-weight:bold;"> Payments: </span><u><span style="color:#09F"><?if ($nights_last_payment==0){ echo ($contador-1);}else{echo $contador;}?></span></u></p>
				  ...
				  <p style="text-align:leftt; font-size:11px;">
                            ...
                            <? if(($info_book['status']==26)||($info_book['status']==27)||($info_book['status']==28)||($info_book['status']==29)){?>
                                 <span style="color:#09F">
							      <? if ((strtotime($ending_date))<=(strtotime(date("Y-m-d")))){?>
							      	<input type="radio" name="status" value="29" checked="checked">Checked out/Long
							      <?}elseif((strtotime($starting_date))==(strtotime(date("Y-m-d")))){?>
								     <input type="radio" name="status" value="26" <? if ($info_book['status']==26) echo "checked=\"checked\"";?> <? if (($info_book['status']!=26)&&($info_book['status']!=28)&&($info_book['status']!=27)) echo "checked=\"checked\"";?>>Checking in/Long
								     <input type="radio" name="status" value="28" <? if ($info_book['status']==28) echo "checked=\"checked\"";?>>Confirmed/Long
								     <input type="radio" name="status" value="27" <? if ($info_book['status']==27) echo "checked=\"checked\"";?> >No confirmed/Long
							      <?}elseif((strtotime($starting_date))>(strtotime(date("Y-m-d")))){?>
							      	 <input type="radio" name="status" value="28" checked="checked" <? if ($info_book['status']==28) echo "checked=\"checked\"";?> <? if (($info_book['status']!=28)&&($info_book['status']!=27)) echo "checked=\"checked\"";?>>Confirmed/Long
								     <input type="radio" name="status" value="27" <? if ($info_book['status']==27) echo "checked=\"checked\"";?>>No confirmed/Long
								  <?}elseif((strtotime($starting_date))<(strtotime(date("Y-m-d")))&&(strtotime($ending_date))>=(strtotime(date("Y-m-d")))){?>
				                    <input type="radio" name="status" value="26" checked="checked">Checked in/Long
								  <?}?>
						         </span>
                            <?}else{?>
                                 <span style="color:#09F">
							      <? if ((strtotime($ending_date))<=(strtotime(date("Y-m-d")))){?>
							      	<input type="radio" name="status" value="33" checked="checked">Checked out/short
							      <?}elseif((strtotime($starting_date))==(strtotime(date("Y-m-d")))){?>
								     <input type="radio" name="status" value="30" <? if ($info_book['status']==30) echo "checked=\"checked\"";?> <? if (($info_book['status']!=30)&&($info_book['status']!=31)&&($info_book['status']!=32)) echo "checked=\"checked\"";?>>Checking in/short
								     <input type="radio" name="status" value="32" <? if ($info_book['status']==32) echo "checked=\"checked\"";?>>Confirmed/short
								     <input type="radio" name="status" value="31" <? if ($info_book['status']==31) echo "checked=\"checked\"";?> >No confirmed/short
							      <?}elseif((strtotime($starting_date))>(strtotime(date("Y-m-d")))){?>
							      	 <input type="radio" name="status" value="32" checked="checked" <? if ($info_book['status']==32) echo "checked=\"checked\"";?> <? if (($info_book['status']!=32)&&($info_book['status']!=31)) echo "checked=\"checked\"";?>>Confirmed/short
								     <input type="radio" name="status" value="27" <? if ($info_book['status']==31) echo "checked=\"checked\"";?>>No confirmed/short
								  <?}elseif((strtotime($starting_date))<(strtotime(date("Y-m-d")))&&(strtotime($ending_date))>=(strtotime(date("Y-m-d")))){?>
				                    <input type="radio" name="status" value="30" checked="checked">Checked in/short
								  <?}?>
						         </span>
