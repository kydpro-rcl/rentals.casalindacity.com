	...
	$qty_nights=$o['nights'];
	 ...
	 //***************************FEES*********************************************/
		...
		if($payments){
			...
			foreach ($payments as $k){
				if($k['class']==1){/*if it's a payment*/	
					...
					if($agent_comision[0]){
						$fee_cc=($cc_amont*0.08);
					}else{
						$fee_cc=($cc_amont*0.1);/*antes del [19 may 2016] era 0.10 (10 %) = sin agente; luego 0.06 (6%) si habia comision de agente: A partir de [11 Jul 2017] es 0.08 (8%) si hay Agentes*/
					}
		...
		if($agent_comision[0]){
			...
			//$percent_discounted_no_tax=$percent_discounted-($percent_discounted*(15.254/100));
			$total_per_nights_gral=($total_ls_amount+$total_hs_amount)-($disc_found['discounted']-($disc_found['discounted']*(15.254/100)));
	 ...
	 /***************************start FEES********************************************/
	 $serfm=services_franco($bed=$villa_re[0]['bed'], $nights=$qty_nights);
	 /*echo $bed; echo " - "; echo $nights;
	 print_r($serfm);
	 die();*/
	 /***************************END FEES********************************************/
	 if(strtotime($o['date'])>=(strtotime('2017-04-05'))){/*if booking is created after or on April 5 2017*/ 
		$wear_tear=wear_tear_funds($beds=$villa_re[0]['bed'], $qtynights=$qty_nights);
	 }
     ...
     $pdf->SetFont('','U');
     $pdf->Cell(40,6,'USD -'.number_format($total_fees+$serfm['total']+$wear_tear,2),'R',0,'R',0);
     $pdf->SetFont('');
     $pdf->Ln();
     $pdf->SetFont('','B');
     $pdf->Cell(145,6,'GRAND TOTAL= ','L',0,'R',0);
     $pdf->Cell(40,6,'USD '.number_format($general_total=($itbis+($sub_total-$total_fees-$serfm['total']-$wear_tear)),2),'R',0,'R',0);
     $pdf->SetFont('');
    ...
    // $pdf->Ln();
     ...
     $pdf->Cell(185,0,'','T');
	  ...
	  /*===============================FEE AND COMISSION---------------------------*/
		...
		if($serfm!=""){
			$pdf->Ln(5);
			$pdf->Cell(140,14,'CHARGES AND FEES',0,0);
			$pdf->Cell(25,14,'',0,0);
			$pdf->Cell(20,14,'Total Fee');
			
			
			
			$pdf->Ln(5);
			$pdf->Cell(140,14,'House keeping fee',0,0);
			$pdf->Cell(25,14,'',0,0);
			$pdf->Cell(20,14,'US$ '.(number_format(($serfm['housekeeping']),2)),0,0);
			$pdf->Ln(5);
			/*$pdf->Cell(140,14,'Electricity fee',0,0);
			$pdf->Cell(25,14,'',0,0);
			$pdf->Cell(20,14,'US$ '.(number_format(($serfm['electricity']),2)),0,0);
			$pdf->Ln(5);*/
		if(strtotime($o['date'])>=(strtotime('2017-04-05'))){/*if booking is created after or on April 5 2017*/ 			
			$pdf->Cell(140,14,'Renters insurance coverage',0,0);
			$pdf->Cell(25,14,'',0,0);
			$pdf->Cell(20,14,'US$ '.(number_format(($wear_tear),2)),0,0);
			$pdf->Ln(5);
		 }
			//$serfm['total'];
		}
		/***************************END services fee FEES********************************************/
		//print_r($o);
		if($agent_comision[0]){
			$pdf->Ln(5);
