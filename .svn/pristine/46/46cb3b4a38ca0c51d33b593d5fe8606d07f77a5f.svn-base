<?
		function arreglos_days($start_date, $end_date){
			...
			if($y2>$y1){  /*si los años son diferentes*/
		      for($y=$y1; $y<=$y2; $y++){  /*years*/
		          if($y==$y1){$mes_inicial=$m1;}else{ $mes_inicial='01';} /*si es el primer año inicia el mes de fecha de inicio*/
		          if($y==$y2){$mes_final=$m2;}else{ $mes_final='12';} /*si es el ultimo año termina el mes de la fecha final*/
			...
			}else{/*entonces se presume que es el mismo año*/
		 ...
		 function Occupancy_Rate_villa($idvilla, $start_month_year, $end_month_year){
			...
			$s_month=$sd[0]; $s_year=$sd[1]; $e_month=$ed[0]; $e_year=$ed[1];
             if($s_year==$e_year){                for($i=$s_month; $i<=$e_month; $i++){ /* running months in this year*/                 $mes=$i; $year=$s_year; $id_villa=$idvilla;
                 //buscar el rate de esta villa en estes mes y año
