...
?>
 ...
 /*if($_POST){*/
     ...
     $dias_arreglos_mes=arreglos_days($start_date=$fecha_inicial_mes, $end_date=$fecha_final_mes);*/
       ...
       $db= new getQueries();
	    $data= new DB();
		$result=$db->booking_unpaid($days=15);
		...
		if ($result){
			...
			<tr>
												<td colspan="11" align="center" width="85%" class="blue_light"><!--<p>--><strong>REPORT OF BOOKINGS WITH NOT PAYMENTS </strong><!--</p>-->
												</td>
											</tr>
				    						<tr bgcolor="#a6cdf4">
												...
												<td  class="blue_dark" align="center" style="font-size:10px;"><strong>Paid</strong>
					    						</td>
												<td align="center" class="blue_dark" style="font-size:10px;"><strong>Total</strong></td>
												<td align="center" class="blue_dark" style="font-size:10px;"><strong>Date created</strong>
					    						</td>
				    						</tr>
			<? 
			$total_mes_villas='';
			...
			foreach ($result as $b){
				$montoTotalPagado=$db->amountRef($b['ref'],'1');
				if($montoTotalPagado>0){
					//mark this booking as paid
					$data->update($id=$b['reservaid'], $info=array('paid'=>'1'), $table='reserves');
					//echo $b['reservaid']."($montoTotalPagado)<br/>";
					//echo "<br/>";
					//die();
				}else{
				  ...
				  //$resultado_booking=noches_del_mes_en_booking($array_days_month=$dias_arreglos_mes, $array_booking=$b,$last_date_month=$fecha_final_mes);
					  ...
					  if(($b['status']!=0)&&($b['status']!=5)&&($b['status']!=7)&&($b['status']!=19)&&($b['status']!=20)&&($b['status']!=21)&&($b['status']!=22)&&($b['status']!=23)&&($b['status']!=24)&&($b['status']!=25)){		/*only if it's not owners, cancelled, maintenance*/
						  ...
						  /*
						  if(($b['status']==3)||($b['status']==10)||($b['status']==13)||($b['status']==17)||($b['status']==27)||($b['status']==31)||($b['status']==35)){		//what is unverified
							...
							<td style="font-size:9px" ><? $villa=$db->villa($b['villa']); echo $villa[0]['no'];?></td>
								...
								<td  style="font-size:11px" ><!--<?=$client[0]['email']?>--><?=$montoTotalPagado?></td>
								<td style="font-size:11px; text-align:right;"  ><?=number_format($total_usd_month,2)?></td>
								<td style="font-size:11px; text-align:right;"  > <? echo $b['date']; ?></td>
							</tr>
						 <? 
