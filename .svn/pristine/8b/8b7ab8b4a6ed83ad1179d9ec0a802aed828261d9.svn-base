...
//validate fields
	...
	if($_SESSION['error']){/*send back to correct errors*/
		$location='../m.booking_details.php';
...
/*
		   ...
		   */
		   ...
		   /*********************TAXES**************/
			  if($_SESSION['C']['paynow']=='50'){
			    $L_TAXAMT = ($_REQUEST['TAXAMT']*0.50);/*price of the article*/
			   }elseif($_SESSION['C']['paynow']=='1'){
			   //only charge one night
			     $L_TAXAMT =number_format(($_REQUEST['TAXAMT']/$qtyReal),2);
			   }else{/*charge 100 percent*/
		   ...
		   $$name_it='Service fee';
		   $$amt_it=number_format(($total_for_items+$L_TAXAMT),2);
		   $$amt_it=number_format(((($total_for_items+$L_TAXAMT)*0.0495)+0.30),2);//	SERVICES FEES 5.4% + 0.30 USD ACORDING WITH PAYPAL TEAM;
		   $$qty_it='1';
		   $$desc_it='';
		   $detail_items.="&$name_it=".${$name_it}."&$amt_it=".${$amt_it}."&$qty_it=".${$qty_it};
		   $$num_it=$ref;
		 ...
		 */
		   ...
		   if($_REQUEST['PaymentType']=='CC'){$paymentOption="&LANDINGPAGE=Billing";}else{$paymentOption="&LANDINGPAGE=Login";}
		   
			$nvpstr="&PAYMENTREQUEST_0_SHIPTOCOUNTRYCODE=US&ADDRESSOVERRIDE=0".$detail_items."&AMT=".(string)$amt."&ITEMAMT=".(string)$itemamt."&NOSHIPPING=1&TAXAMT=".$L_TAXAMT.$numb_desc.$paymentOption."&ReturnUrl=".$returnURL."&CANCELURL=".$cancelURL ."&CURRENCYCODE=".$currencyCodeType."&PAYMENTACTION=".$paymentType;
