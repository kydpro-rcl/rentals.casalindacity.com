...
class Database1 {
	...
	public function Database1() {
		$this->connection = new mysqli(SERVER1, USER1, PASS1, DB1);
		/* check connection */
		if (mysqli_connect_errno()) {
			Logger1::logEvent(Logger1::LOG_ERROR, basename(__FILE__), __LINE__, "Database Connect failed: ".mysqli_connect_error());
			exit(-1);
		}
	}
	...
	// Escape a field
	public function myesc($string) {		$string_clean=trim($string);
...
//echo "\n\n!!! ";
			Logger1::logEvent(Logger1::LOG_ERROR, basename(__FILE__), __LINE__, "Query failed: ".$this->connection->errno.":".$this->connection->error);
			return false;
		}
	}
