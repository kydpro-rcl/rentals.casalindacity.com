<?
 if ($_SESSION['info']){
   if ($_POST){
	...
	?>
	    <h2 style="text-align:center; color:#06C;"><? if($_SESSION['apolo']==3){ ?>Apollo <?}else{?>Try and buy renters - Step 2<?}?></h2>
	<?php 
	} 
	?>
		...
		/* END	NEW PROMOTION TIPE */
			...
			<!--//   codigo para promotion code//-->
	 	     <hr />
	      <form method="post" action="short-term-TB3.php" name="st">
	    ...
	    <!--TERMINA NUEVO CODIGO PARA LOS SERVICIOS-->
			   <tr>
	            ...
	            <!--SERVICES ADDITIONALS-->
					<tr>
						<td id="td" style="text-align:right;"><strong>TOTAL <?  if($_POST['car']){?>WITHOUT CARS<?}?> = </strong></td>.
						<td id="td" style="text-align:right;"><? $general_amount=$sub_total+$itbis; echo "<strong>US$ ".number_format($general_amount-$amount_discounted,2)."</strong>"; ?></td>
					</tr>
					<?php
					...
					?>
					<tr>
						<td id="td" style="text-align:right; color:red;"><strong>TRY AND BUY CLIENT = </strong></td>.
						<td id="td" style="text-align:right; color:red;"><?  echo "<strong>US$ ".number_format($tbtotal,2)."</strong>"; ?></td>
					</tr>
					<tr>
						<td id="td" style="text-align:right;color:green;"><strong>NEGUEN <?  if($_POST['car']){?>WITHOUT CARS<?}?> = </strong></td>.
						<td id="td" style="text-align:right;color:green;"><? echo "<strong>US$ ".number_format($general_amount-$amount_discounted-$tbtotal,2)."</strong>"; ?></td>
					</tr>
				</table>
				<?php
				if($mid_term_booking=='1'){ 
...
<div style="float:left; width:450px; border-bottom:0px solid blue;">   <!--START SECOND DIV DETAILS-->
	           ...
	           <tr><td id="td" colspan="4"> <strong>Reservation Comment:</strong><br /><span style="color:#3b3838;"> <?=wordwrap($comment, 75, "\n", true);?> </span><?}?></td></tr>
				...
				switch ($status){
					case 38: echo "<span style='color:green;'>TB Checked in</span>";
						break;
					case 39: echo "<span style='color:#00F;'>TB No Confirmed</span>";
					        /*echo "&nbsp;-&nbsp;<span style='color:green;'>Deposit</span>&nbsp;<input type=text name=deposit size=5 value=000.00>";*/
						break;
					case 40: echo "<span style='color:#FF0000;'>TB Confirmed</span>";
							//echo " - <span style='color:green;'>Deposit</span> <input type=text name=deposit size=5 value=000.00>";
						break;
					case 41: echo "<span style='color:#FF0000;'>TB Checked out</span>";
							break;
				}
...
<div style="clear:both; width:100%; border-bottom:0px solid black;">   <!--START THIRD DIV DETAILS-->
	...
	<input type="hidden" name="tbt" value="<?=$_POST['tbtype'];?>" />
	<input type="hidden" name="tbp" value="<?=$_POST['tbprice'];?>" />
	...
	<?}else{?>
		 <INPUT class="book_but" TYPE="button" onClick="location.href='short-term-book.php?villa=<?=$villa_id?>&v2=<?=$villa_id?>&start=<?=$starting?>&end=<?=$ending?>&destine=TB'"  VALUE="Back" title="Hit to go back" />
<?	}	?>
