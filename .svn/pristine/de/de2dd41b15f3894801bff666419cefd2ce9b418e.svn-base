...
require_once('../../booking/init.php');
	 ...
	 /*print_r($v);
	die();*/
	require_once('../../photos/functions.php');//connect to pictures db/table
	$db = new Basededatos2();//class picture db/table
 ...
 if(($v['able_r']==1)&&($v['vonline']==0)){
	...
	if($server_name=='http://localhost'){
		$add_forlder='/rentals.casalindacity.com';
	}else{
		$add_forlder='';
	}
	
	$main_pic_folder=$add_forlder.'/photos/'.$v['id'].'/ha/';
	$all_pic_folder=$add_forlder.'/for_rent/tor_pics/photos/villa'.$v['no'].'/full/';
	...
	echo '<listing>';
		...
		echo '<adContent>';
			...
			echo '<description>';
				echo '<texts>';
					echo '<text locale="en">';
						echo '<textValue>'.htmlentities($v['head']).'</textValue>';
					echo '</text>';
				echo '</texts>';
			echo '</description>';
			echo '<headline>';
				echo '<texts>';
					echo '<text locale="en">';
						echo '<textValue>'.htmlentities($v['headline']).'</textValue>';
					echo '</text>';
				echo '</texts>';
			echo '</headline>';
		...
		echo '<location>';
			echo '<address>';
				...
				echo '<country>DO</country>';
				echo '<postalCode>57000</postalCode>';
			echo '</address>';
			echo '<description>';
			echo '	<texts>';
					echo '<text locale="en">';
						echo '<textValue></textValue>';
					echo '</text>';
				echo '</texts>';
			echo '</description>';
		...
		if(!empty($images)){
			foreach($images as $row){
				//echo '<img src="'.$_GET['id'].'/'.$row['img_name'].'" alt="">';
				echo '<image>';
					echo '<externalId>'.$row['id'].'1</externalId>';
					echo '<title>';
						echo '<texts>';
							echo '<text locale="en">';
								echo '<textValue></textValue>';
						...
						echo '</texts>';
					echo '</title>';
					echo '<uri>'.$server_url.$main_pic_folder.rawurlencode($row['img_name']).'</uri>';
				echo '</image>';
			}
		}
		/*
			echo '<image>';
				echo '<externalId>'.$v['no'].'1</externalId>';
				echo '<title>';
					echo '<texts>';
						echo '<text locale="en">';
							echo '<textValue></textValue>';
					...
					echo '</texts>';
				echo '</title>';
				echo '<uri>'.$server_url.$main_pic_folder.$v['pic'].'</uri>';
			echo '</image>';
		if ($fotos){
			$foto_orden=1;
			...
			echo '<image>';
				echo '<externalId>'.$v['no'].$foto_orden.'</externalId>';
					echo '<title>';
						...
						echo '</texts>';
					echo '</title>';
					echo '<uri>'.$server_url.$all_pic_folder.$image.'</uri>';
			echo '</image>';
			}
		}*/
		echo '</images>';
