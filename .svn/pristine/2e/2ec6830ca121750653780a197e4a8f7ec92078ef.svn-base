...
<?
	...
	$bookings_found=$data->bookings_referal_last3months($k ['id']);//show bookings per referals
	$total_records=$data->getAffectedRows();
	?>
	
	<td id='td'><span style="font-size:10px;"><?=$total_records?> <?php/*=$bookings_found*/?></span></td>
<td id='td'><span style="font-size:10px;"><? if($bookings_found[0]){ echo(date('Y-m-d', strtotime($bookings_found[0]['date']))); }?></span></td>
</tr>
<?php
}
?>
