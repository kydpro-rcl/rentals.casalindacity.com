...
require_once 'const.php';
    ...
    class PaypalInvoiceAPI
    {
		...
		//if(defined('API_USERNAME'))
		 private $API_User=API_USERNAME;

		//if(defined('API_PASSWORD'))
		  private $API_Pass=API_PASSWORD;

		//if(defined('API_SIGNATURE'))
		  private $API_Signat=API_SIGNATURE;
        /**
         *
         * @param string $md - live/sandbox
         */
        function __construct($md = "sandbox")
        {
            ...
            if($md == "live")
            {
                 ...
                 */
                $this->APPLICATION_ID = "APP-0A2494799M707451U";
        ...
        /**
         *
         * @global <type> $API_Username - Username of Paypal API
         * @global <type> $API_Password - Password of Paypal API
         * @global <type> $Signature - Signature of Paypal API
         * @return array - Array contains the complete setting of Paypal Invoice API Header
         */
        function getInvoiceAPIHeader()
        {		
			// echo $API_User; echo "<br/>"; echo $API_Pass; echo "<br/>"; echo $API_Signat;
			 // echo API_USERNAME; echo "<br/>"; echo API_PASSWORD; echo "<br/>"; echo API_SIGNATURE;
            global $API_User, $API_Pass, $API_Signat;
            $headers[0] = "Content-Type: text/namevalue";               // either text/namevalue or text/xml
            $headers[1] = "X-PAYPAL-SECURITY-USERID: {$this->API_User}";    //API user
            $headers[2] = "X-PAYPAL-SECURITY-PASSWORD: {$this->API_Pass}";  //API PWD
            $headers[3] = "X-PAYPAL-SECURITY-SIGNATURE: {$this->API_Signat}";    //API Sig
        ...
        function prepareCreateInvoice($aryData, $aryItems)
        {
            ...
            for($cnt=0;$cnt<$nLoop;$cnt++)
            {
                if(trim(@$aryItems[$cnt]['name'])!= "")
                    $aryCreateInvoice["invoice.itemList.item($cnt).name"]       = $this->adjustStringLength($aryItems[$cnt]['name']);
                if(trim(@$aryItems[$cnt]['description'])!= "")
                    $aryCreateInvoice["invoice.itemList.item($cnt).description"]= $this->adjustStringLength($aryItems[$cnt]['description']);
                ...
                if(trim(@$aryItems[$cnt]['quantity'])!= "")
                    $aryCreateInvoice["invoice.itemList.item($cnt).quantity"]   =$this->adjustStringLength($aryItems[$cnt]['quantity']);
                if(trim(@$aryItems[$cnt]['unitprice'])!= "")
                    $aryCreateInvoice["invoice.itemList.item($cnt).unitPrice"]  = $this->adjustStringLength($aryItems[$cnt]['unitprice']);
