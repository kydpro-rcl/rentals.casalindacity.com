 ...
 if ($_SESSION['info']){
   if ($_POST){   	 $_SESSION['NO_REFRESH']=1;
	...
	<!--<img src="images/logo.gif" border="0" style="float:left"><br/>  -->
	    <h2 style="text-align:center; color:#06C;">Short Term BUYER - Step 2</h2>
			...
			<!--//   codigo para promotion code//-->
	 	     <hr />
	      <form method="post" action="short-term-buyer3.php" name="st">
	...
	<table width="400" style="border-bottom:1px solid #060; border-top:1px solid #060; border-left:1px solid #060; border-right:1px solid #060;">
	    ...
	    <!--TERMINA NUEVO CODIGO PARA LOS SERVICIOS-->
			   ...
			   <tr>
	            ...
	            <tr><td id="td" style="text-align:right;"><strong>TOTAL <?  if($_POST['car']){?>WITHOUT CARS<?}?> = </strong></td><td id="td" style="text-align:right;"><? $general_amount=$sub_total+$itbis; echo "<strong>US$ ".number_format($general_amount,2)."</strong>"; ?></td></tr></table>
...
<div style="float:left; width:450px; border-bottom:0px solid blue;">   <!--START SECOND DIV DETAILS-->
	           ...
	           <tr><td id="td" colspan="4"> <strong>Reservation Comment:</strong><br /><span style="color:#3b3838;"> <?=wordwrap($comment, 75, "\n", true);?> </span><?}?></td></tr>
				...
				switch ($status){
					case 30: echo "<span style='color:green;'>Checking in</span>";
						break;
					case 32: echo "<span style='color:#00F;'>Confirmed</span>";
					        echo "&nbsp;-&nbsp;<span style='color:green;'>Deposit</span>&nbsp;<input type=text name=deposit size=5 value=000.00>";
						break;
					case 31: echo "<span style='color:#FF0000;'>Not confirmed</span>";
							//echo " - <span style='color:green;'>Deposit</span> <input type=text name=deposit size=5 value=000.00>";
						break;
					case 33: echo "<span style='color:#FF0000;'>Check out</span>";
							break;
				}
...
<div style="clear:both; width:100%; border-bottom:0px solid black;">   <!--START THIRD DIV DETAILS-->
	...
	<!--campos que envian datos a la seccion final-->
	    <INPUT class="book_but" TYPE="button" onClick="location.href='short-term-book.php?villa=<?=$villa_id?>&v2=<?=$villa_id?>&start=<?=$starting?>&end=<?=$ending?>&destine=Buyers'"  VALUE="Back" title="Hit to go back">
