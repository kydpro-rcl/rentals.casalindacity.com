...
<? if(!$_GET['y']){?>
<form method="post" action="stat_from2.php" >
	<p id="fields" style="text-align:center;">From:
	    <input type="text" name="from" value="<?=$_POST['from']?>" placeholder="YYYY-MM-DD"/>
      To:
    <input type="text" name="to" value="<?=$_POST['to']?>" placeholder="YYYY-MM-DD"/>
...
<table  align="center" cellpadding="0" cellspacing="1" border="0" style="font-weight:normal; font-family:arial;">
            ...
            // $fecha_inicial_mes=$this_year.'-'.$this_month.'-01';
			  $fecha_final_mes=$_POST['to'];
             $fecha_inicial_mes=$_POST['from'];
			...
			$days_qty=daysDifference2($fecha_final_mes, $fecha_inicial_mes);
			$ultimo_dia=$days_qty;/*cantidad de dias totales para calcular rentable nights*/
			...
			<?
                ...
                $arr_totals['total']['3bdr']['rate']=($arr_totals['total']['3bdr']['occ']/$arr_totals['total']['3bdr']['rent'])*100;
                 $arr_totals['total']['5bdr']['rate']=($arr_totals['total']['5bdr']['occ']/$arr_totals['total']['5bdr']['rent'])*100;
...
<table border="1" cellspacing="0" cellpadding="1" style="border:1px solid black; font-size:10px;background-color:orange;">
	<tr>
		<td colspan="3" align="center"><span style="text-transform:uppercase;">Rental from <?=$_POST['from']?> to <?=$_POST['to']?>
             </span></td>
...
<?
 /*===================AÑO ANTERIOR=============================*/
   # $ly=statFrom2($fecha_inicial_mes, $fecha_final_mes);/*SE DEBE LLAMA LA MISMA FECHA EL ANO ANTERIOR*/
