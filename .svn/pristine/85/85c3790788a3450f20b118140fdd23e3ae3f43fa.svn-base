	...
	if($ack == 'SUCCESS' || $ack == 'SUCCESSWITHWARNING'){
		 ...
		 if(!$cant_new>0){
			...
			//------varibles para el cliente----//
				...
				$referQry= new getQueries();
                 $referral_details=$referQry->show_any_data_limit1('commission', 'id', $_SESSION['agent_id'], '=');  //get datails for this referral
		            ...
		            if ($_SESSION['REFERRALID']>0){
						    ...
						    $referido_anterior=$link->show_any_data_limit1('bookingreferred', 'ref_book', $ref, '=');
							  ...
							  if ($referido_anterior){
								...
								$result=$db->Assign_a_booking($ref, $_SESSION['REFERRALID'], $id_adm, $amountdiscountedComission=0);
							  }
				    }
		  ...
		  /*=====================================================================================================================*/
			...
			//sent notification to referral.
			$bodyReferral.='Dear '.$referral_details[0]['name']." ".$referral_details[0]['lastname'].'\n';
			$bodyReferral.="A client has created a new booking with number: $ref\n";
			@sendMailText($bodyReferral, $address=$referral_details[0]['email'], $subject="Booking No.:".$ref, $from_add='rentalmanager@casalindacity.com', $from_name='RCL Rental Department');
		}
	}
 }
?>
