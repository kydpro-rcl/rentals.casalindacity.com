...
if ($_SESSION['info']){
 ...
 if ($_POST['ref']!=''){
    ...
    $o=$busy[0];
	 ...
	 $montoTotalSTransferido=$db->amountRef($_POST['ref'],'5');/*Move to another booking*/
	...
	/* echo $owner[0]['company'];
	 echo "<br/>";
	 echo $owner[0]['name'];
	 echo " ";
	 echo $owner[0]['lastname'];
	 echo "<br/>";
	 echo $villa_re[0]['no'];
	 echo "<br/>";
	 echo $owner[0]['RNC'];	   
	...
	//Cabecera de p�gina
		function Header()
		{
		 ...
		 # $this->Cell(60,9,$propiet);
		  ...
		  #$this->Ln(1);
		  # $this->Cell(87);
			#$this->Cell(87,33,'Sosua, Republica Dominicana');
		   #$this->Ln(20);
		  // $this->Cell(82);
		...
		function FacturaService($header,$data)
			...
			$fill=0;
				foreach($data as $row)
				{
				   ...
				   $this->Cell($w[0],6,substr($row['comment'],0,45)." (".$row['qty']."x".$row['unit']."$taxInfo)",'LR',0,'C',$fill);
					$this->Cell($w[1],6,'USD '.$row['unit'],'LR',0,'R',$fill);
					$price=$row['qty']*$row['unit'];
					$this->Cell($w[2],6,'USD '.$price,'LR',0,'R',$fill);
	...
	*/
	...
	/*================================================================================*/
		  ...
		  if($owner[0]['company']!=''){
			  //$pdf->Cell(60);
			  $pdf->Cell(60,9,$owner[0]['company']);
			   //$this->Image('images/invoice/letters.jpg',95,18,40);
			  $pdf->Ln(1);
		  }else{
			   //$pdf->Cell(60);
			   $pdf->Cell(60,9,$owner[0]['name'].' '.$owner[0]['lastname']);
			   $pdf->Ln(1);
			  
		  }
		  
		  // $pdf->Cell(60);
		   $pdf->Cell(70,20,'Villa No. '.$villa_re[0]['no']);
		  $pdf->Ln(1);
		   $pdf->SetFont('Times','',8);
		  ...
		  if($owner[0]['RNC']!=''){ 
		   //$this->Ln(1);
		   //$pdf->Cell(60);
		   $pdf->Cell(60,30,'RNC:'.$owner[0]['RNC']); 
		   $pdf->Ln(1);
		   //$pdf->Cell(60);
			$pdf->Cell(60,33,'Sosua, Republica Dominicana');
		   $pdf->Ln(20);
		  }else{
			//$pdf->Cell(60);
			$pdf->Cell(60,30,'Sosua, Republica Dominicana');
		   $pdf->Ln(20);
		  }
    ...
    if((strtotime($o['date']))<(strtotime(TAX_FECHA))){/* si la fecha creada fue menor al 1 de enero 2013 solo aplica 16%*/
     ...
     $pdf->Cell(40,6,'USD '.number_format($general_total=($itbis+$sub_total),2),'R',0,'R',0);
	  ...
	  if (($o['status']==38)||($o['status']==39)||($o['status']==40)||($o['status']==41)){
		 $traynbuyinfo=$db->tbuy($ref=$_POST['ref']);
		 $pdf->SetFont('');
		 $pdf->Ln();
		 $pdf->Cell(145,6,'PAYABLE TO TB CLIENT= ','L',0,'R',0);
		 $pdf->Cell(40,6,'USD '.number_format($traynbuyinfo['total'],2),'R',0,'R',0);
		 $pdf->Ln();
		 $pdf->Cell(145,6,'PAYABLE TO NEGUEN= ','L',0,'R',0);
		 $pdf->Cell(40,6,'USD '.number_format($general_total-$traynbuyinfo['total'],2),'R',0,'R',0);
	  }
     ...
     $pdf->Cell(40,6,'USD '.number_format((($general_total+$total_per_services+$total_per_excursions)-$o['dep']-$montoTotalPagado+$montoTotalDevuelto+$montoTotalSTransferido),2),'R',0,'R',0);
     $pdf->SetFont('');
	 $pdf->Ln();
	 ...
	 if($montoTotalSTransferido>0){
	  $pdf->Cell(145,6,'Trasferred to booking','L',0,'R',0);
      $pdf->Cell(40,6,'USD '.number_format($montoTotalSTransferido,2),'R',0,'R',0);
     $pdf->Ln();
	 }
     $pdf->Cell(185,0,'','T');
	 ...
	 if (($o['status']==34)||($o['status']==35)||($o['status']==36)||($o['status']==37)){
		...
		//$pdf->Ln();
		if(strtotime($o['date'])>=(strtotime('2017-04-01'))){/*if booking is created after or on April 1st 2017*/
			switch($villa_re[0]['bed']){
				...
				default: $kilowatts=25; break;
			}
			$pdf->Ln(3);
			
			$pdf->MultiCell(0,5,'NOTE: Normal use of electricity '.$kilowatts.' KW per day. is included in the rate. Upon arrival and departure, a reading of the electricity meter will be done. Excessed use of electricity will be charged extra upon departure.',0,L);
			//$pdf->Cell(93,6,'NOTE: There is an electricity consumption credit of 25 KW per day. Additional electricity charges will apply if exceed the credit.',0,0,'L',0);
		}
	}
     ...
     $pdf->Cell(93,6,'____________________________________',0,0,'C',0);
     $pdf->Ln();
     $pdf->Cell(93,6,'Receptions',0,0,'C',0);
	 ...
	 //////////////////////////////////////////INICIO PAGINA PARA LA ELECTRICITY//////////////////////////////////////////////////////	 
	$pdf->AddPage();
	$pdf->SetTitle('Residencial Casa Linda-Invoice');
	$pdf->SetFont('Times','',16);
		  $pdf->SetFont('Arial','B',15);
	...
	$pdf->SetFont('Times','',12);$pdf->SetFont('','B');
	$pdf->Cell(80,10,'ELECTRICITY CONSUMPTION DETAILS');      //invoice title
	...
	$pdf->Ln(20);
	 ...
	 /*NOTE OF ELECTRICITY ONLY IF MID TERM RENTAL*/
			...
			$includekw=$kilowatts*$o['nights'];
			
			$pdf->Ln(3);
			$pdf->Cell(75,6,'Meter starts at: __________________KW',0,0,'L',0);
			$pdf->Ln(5);
			$pdf->Cell(68,6,'Meter ends at: ____________________KW',0,0,'L',0);
			$pdf->Ln(5);
			$pdf->Cell(68,6,'Total consumption: ____________________KW',0,0,'L',0);
			$pdf->Ln(10);
			$pdf->Cell(68,6,'Total electricity included on this booking is: '.$includekw.' KW',0,0,'L',0);
			$pdf->Ln(10);
     ...
     // $pdf->Cell(93,6,'Customer Signature',0,0,'C',0);
      ...
      //////////////////////////////////////////FINAL PAGINA PARA LA ELECTRICITY//////////////////////////////////////////////////////	 
     ...
     if (!empty($servicios_reserva)){
        ...
        foreach ($servicios_reserva as $s){
			 if($s['type']=='Car_Rental'){$total_por_vehiculos=$s['price'];}
			 if($s['tax']>0){
				 $services_tax2+=(($s['unit']*$s['qty'])*$s['tax']);
			 }
			 $sub_total+=$s['price'];
		 }

	    // $sub_total=($o['aps']);
	     $pdf->Cell(40,6,'USD '.number_format($sub_total-$services_tax2,2),'R',0,'R',0);
	     $pdf->SetFont('');
	     $pdf->Ln();
	     $pdf->Cell(145,6,'ITBIS (VAT - TAXS) '.TAX_PERCENT.' = ','L',0,'R',0);
	     $pdf->SetFont('','U');
	     ...
	     $pdf->Cell(40,6,'USD '.number_format($services_tax+$services_tax2,2),'R',0,'R',0);
	     $pdf->SetFont('');
	     $pdf->Ln();
	     $pdf->SetFont('','B');
	     $pdf->Cell(145,6,'TOTAL PER SERVICES= ','L',0,'R',0);
	...
	if (($o['status']!=0)){
		...
		/************************* INICIA FACTURA PARA CONTABILIDAD CON LOS SERVICIOS REDUCIDOS *************************************/
			$pdf->AddPage();
			/*================================================================================*/
			  $pdf->SetFont('Arial','B',15);
		...
		$pdf->SetFont('Times','',12);$pdf->SetFont('','B');
		$pdf->Cell(80,10,'INVOICE FOR ACCOUNTING');      //invoice title
		...
		$serfm=services_franco($bed=$villa[0]['bed'], $nights=$o['nights']);

		if($serfm!=''){    //do this only if any amount for services
			 $pdf->Ln();
			 $pdf->Cell(145,6,'Housekeeping =','L',0,'R',0);
			 $pdf->Cell(40,6,'USD '.number_format(($serfm['housekeeping']),2),'R',0,'R',0);
			  /* $pdf->Ln();
			$pdf->Cell(145,6,'Electricity =','L',0,'R',0);
			 $pdf->Cell(40,6,'USD '.number_format(($serfm['electricity']),2),'R',0,'R',0);*/
			
		}

		if($serfm!=''){    //do this only if any amount for services
			$pdf->Ln();
			$pdf->Cell(145,6,'Laundry =','L',0,'R',0);
			$pdf->Cell(40,6,'USD '.number_format(($serfm['laundry']),2),'R',0,'R',0);
			/* $pdf->Ln();
          $pdf->Cell(145,6,'Electricity =','L',0,'R',0);
           $pdf->Cell(40,6,'USD '.number_format(($serfm['electricity']),2),'R',0,'R',0);*/
		...
		if(strtotime($o['date'])>=(strtotime('2017-04-05'))){/*if booking is created after or on April 5 2017*/ 
				 $pdf->Ln();
				 $pdf->Cell(145,6,'Renters insurance coverage =','L',0,'R',0);
				 $wear_tear=wear_tear_funds($beds=$villa[0]['bed'], $qtynights=$o['nights']);
				 $pdf->Cell(40,6,'USD '.number_format(($wear_tear),2),'R',0,'R',0); 
			}
			/*print_r($serfm);
			die();*/
		 $pdf->Ln();
		 $pdf->SetFont('','B');
		 $pdf->Cell(145,6,'NET TOTAL= ','L',0,'R',0);
		 //$pdf->Cell(40,6,'USD '.number_format($net_total=($sub_total-$serfm['total']-$wear_tear),2),'R',0,'R',0);
		 $pdf->Cell(40,6,'USD '.number_format($net_total=($sub_total-$serfm['total']-$wear_tear),2),'R',0,'R',0);
		 $pdf->SetFont('');
		  //----------amount for excrusions----------------------
			$total_per_excursions=0;
		...
		/* if($total_per_excursions>0){    //do this only if any amount for services
		 ...
		 $pdf->Cell(93,6,'Receptions',0,0,'C',0);
		  ...
		  $pdf->Cell(93,6,$pagable_para,0,0,'C',0);*/
		/****************************** TERMINA FACTURA PARA CONTABILIDAD CON LOS SERVICIOS REDUCIDOS *******************************/	
	}
