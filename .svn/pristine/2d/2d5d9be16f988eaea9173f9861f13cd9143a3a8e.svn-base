   ...
   if ($_SESSION['info']['level']==1){
		...
		if ($_POST['id']) $_GET['id']=$_POST['id']; //get id number for this villa
			...
			//datos del arhivo
			  if ($HTTP_POST_FILES['photo']['name']!=''){
				...
				//compruebo si las características del archivo son las que deseo
	         	...
	         	$link= new getQueries();
	            $owner=$link->show_id('owners', $_GET['id']);
	          	$photo=$owner[0]['photo']; //src is was it was
	          }
			  ...
			  if ($HTTP_POST_FILES['rent']['name']!=''){
				...
				//compruebo si las características del archivo son las que deseo
					if (!((strpos($tipo_archivo2, "pdf") || strpos($tipo_archivo2, "PDF")) && ($tamano_archivo2 < 500000))) {
					   $_GET['error']['rent']="should be .pdf and size 500 KB max.";
	         	...
	         	$link= new getQueries();
	            $owner=$link->show_id('owners', $_GET['id']);
	          	$rent_contract=$owner[0]['contract_rent']; //src is was it was
	          }
			  ...
			  if ($HTTP_POST_FILES['services']['name']!=''){
				...
				//compruebo si las características del archivo son las que deseo
					if (!((strpos($tipo_archivo3, "pdf") || strpos($tipo_archivo3, "PDF")) && ($tamano_archivo3 < 500000))) {
					  $_GET['error']['service']="should be .pdf oand size 500 KB max.";
	         	...
	         	$link= new getQueries();
	            $owner=$link->show_id('owners', $_GET['id']);
	          	$serv_contract=$owner[0]['contract_serv']; //src is was it was
	          }
	     ...
	     //end valitation
			...
			//SOLO SE INTRODUCEN SI TODO ESTA BIEN
	         if (!$_GET['error']){ // if not error save data
		      //introducir aqui en la base de datos
		        $db= new subDB();
		        #$datos=$db->UpdateOwner($_GET['id'], $username, $password, $name, $lastname, $email, $phone, $_POST['movil'], $fax, $cedula, $passport, $language, $zip, $address, $country, $photo, $rent_contract, $serv_contract, $comentario, $active, $_SESSION['info']['id'],$_POST['cedula2'],$_POST['passport2']);
			...
			$datos=$db->update($id=$_GET['id'], $fields, $table);	
			/*=============================================================================================================*/
