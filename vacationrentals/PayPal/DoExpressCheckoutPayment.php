		 ...
		 if(!$cant_new>0){
			...
			if(!$_SESSION['customer_id']){
				...
				if ($qty_nights>=$price_settings['mid_m_night']){ 
					$status="36"; /*---Confirmed mid*/
				}else{
					$status="2"; /*---Confirmed short*/
				}
				...
				//@$id_reserve=$db->insert_short_reserva_online($ref, $id_ocupacion, $id_customer, $adults_qty, $children_qty, $interm_id, $qty_nights, $HS_nights, $LS_nights, $price_per_night, $priceHS, $amount_commision, $sub_total_rent, $ITBIS, $services_amount, $deposit, $general_amount, $status, $reserve_comment, "1");  //INSERT RESERVE AND TAKE IT ID
				 ...
				 $reserv=array( 'ref'=>$ref,
								...
								'hearabout'=>$_SESSION['C']['ha']);
								
				$id_reserve=$db->insert_id($reserv, $table='reserves');
				...
				$children_qty=$_SESSION['C']['k'];
					...
					if ($children_qty>0){    //si la cantidad de niños sobrepasa 1 entonces son insertado en la base de datos
					  for ($c=1;$c<=$children_qty; $c++){
					  	...
					  	$lastname='';
						$name=$_SESSION['k'][$c];
						switch($_SESSION['ka'][$c]){
							case 6: $lastname='(0-6)'; break;
							case 12: $lastname='(7-12)';break;
							case 18:$lastname='(13-17)'; break;
						}
						//$lastname=$_SESSION['ka'][$c];
...
ga('ecommerce:addTransaction', {
  'id': '<?=$ref;?>',
  'affiliation': 'reservation: <?=$ref;?>',
  'revenue': '<?=$general_amount?>',
  //'shipping': '5',
  'tax': '<?=$impuestos?>',
...
ga('ecommerce:addItem', {
    ...
    //'sku': 'DD23444',
    'category': 'Rentals',
    'price': '<?=($general_amount/$qty_nights);?>',
    'quantity': '<?=$qty_nights;?>',
    'currency': 'USD' // local currency code.
  });
...
<body>
<div class=\"container\">
<p><img class=\"img-responsive\" src=\"https://rentals.casalindacity.com/vacationrentals/images/logo.png\" alt=\"www.casalindacity.com\" title=\"www.casalindacity.com\" /></p>
...
<p>  <br>
  You will be required to provide passports or valid government ID for all adults in your party at check in.
Be prepared to provide a security deposit of $75 USD per bedroom, this will be done via a hold on a credit card only (NO CASH DEPOSITS).
<br>
</p>
<p>Our office is located at the second gate to the right on El Choco Road.  </p>
<p>  Upon arrival you will receive all keys and controls for the villa.
  <br>
 There will also be an inventory list of what was in your villa upon check-in from our staff, please go through this as you are responsible for any breakage in the villa during your stay. 
  <br>
...
<p>Residencial Casa Linda strives to create the best relaxed and enjoyable vacations for all of our clients.<br>
 Therefore please respect our rules regarding parties and loud noises during your stay, more info will be give upon arrival.

</p>
<p>Daily housekeeping and pool service is included in your villa as well as Free shuttle bus from Sosua to Cabarete on a schedule.
</p>


<ul>
  <li>18% tax will be added to all nightly rental rates</li>
...
2-bedroom: 25 kW, 3-bedroom: 30 kW, 4-bedroom: 40 kW, 5-bedroom: 50 kW, 6-bedroom: 50 KW
</li>
  ...
  <p>
  Don\'t miss out on our extra services we offer, airport pickup, excursions, car rental and more! Please contact our concierge at travelplanner@casalindacity.com 
<br>
...
<small><a href=\"https://rentals.casalindacity.com/vacationrentals/terms-conditions.php\" target=\"new\">Terms and Conditions </a></small></strong></p>
</div>
</body>
</html>";
