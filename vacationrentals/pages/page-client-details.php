...
/*if ($_POST['continue']){*/
	...
	if($_POST){
		...
		unset($_SESSION['adults']);if($_POST['adults']){$_SESSION['adults']=$_POST['adults'];}else{$_SESSION['adults']=1;}
		unset($_SESSION['kids']); if($_POST['kids']){$_SESSION['kids']=$_POST['kids'];}else{$_SESSION['kids']=0;}
	}
	if($_POST['agent']!=''){
		unset($_SESSION['agent_id']); $_SESSION['agent_id']=$_POST['agent'];
	}
	unset($_SESSION['villa_details']);
		$informacion_villa=$db->singleVilla4rent($_SESSION['villa']);
		$_SESSION['villa_details']=$informacion_villa[0];
		
		if(!$informacion_villa){
...
<div class="container">
  <h3 style="color:#06F;">BOOKING DETAILS:
    </h3>
<div class="row">
  <div class="col-md-3"><strong>Villa No.
    <?=$_SESSION['villa_details']['no']?> ( <?=$_SESSION['villa_details']['bed']?> Bedrooms)</span></strong> </div>
   <div class="col-md-3"><strong>From: <span style="color:#cc1c0a; text-transform:uppercase;">   <?=date('M j, Y',strtotime($_SESSION['desde']))?> </span> To: <span style="color:#cc1c0a; text-transform:uppercase;"> <?=date('M j, Y',strtotime($_SESSION['hasta']))?> </span></strong></div>
  <div class="col-md-3"><strong>Total: US$ <?=number_format($_SESSION['total'],0)?> (+ 18% tax)</strong></div>
  <div class="col-md-3"><strong>Guests: <?=$_SESSION['adults']?> Adults - <?=$_SESSION['kids']?> Children</strong></div>
</div>
  ...
  <form name="new_villa" method="post"  action="PayPal/ReviewOrder.php">
    ...
    <div class="row">
      ...
      <div class="col-md-3">
        <div class="form-group">
          ...
          <select class="form-control" name="hear_about" required="required">
		    <option value="">Select</option>
            <option value="1">Facebook Ad</option>
            <option value="2">Google Ad</option>
            <option value="3">Website search for vacation homes/rentals</option>
	...
	<?php
				switch($_SESSION['villa_details']['bed']){
					case 2:
						//$max_child=4;
						if($_SESSION['adults']>4){$_SESSION['adults']=4;}
						$max_adult=4;
						break;
					case 3:
						//$max_child=6;
						if($_SESSION['adults']>6){$_SESSION['adults']=6;}
						$max_adult=6;
						break;
					case 4:
						//$max_child=8;
						if($_SESSION['adults']>8){$_SESSION['adults']=8;}
						$max_adult=8;
						break;
					case 5:
						//$max_child=10;
						if($_SESSION['adults']>10){$_SESSION['adults']=10;}
						$max_adult=10;
						break;
					case 6:
						//$max_child=12;
						if($_SESSION['adults']>12){$_SESSION['adults']=12;}
						$max_adult=12;
						break;
					default:
						//$max_child=4;
						if($_SESSION['adults']>2){$_SESSION['adults']=2;}
						$max_adult=2;
				}
				?>
	<div class="row">		
			...
			<div class="col-md-3">
				<div class="form-group">
					<input class="form-control" type="text" name="a<?=$i?>" placeholder="Adult # <?=$i?> full name" required="required"/>
				</div>
			</div>
			<?}?>
	</div>
	<div class="row">		
			<? for($i=1; $i<=$_SESSION['kids']; $i++){?>
			<div class="col-md-3">
				<div class="row">
					<div class="col-md-8 col-sm-8 col-xs-8">
						<div class="form-group">
						<label for="stateSelect">Child # <?=$i?></label>
						  <input class="form-control" type="text" name="k<?=$i?>" placeholder="Full name" required="required"/>
						</div>
					</div>
					<div class="col-md-4 col-sm-4 col-xs-4">
						<div class="form-group">
							<label for="stateSelect">Age</label>
							<select class="form-control" name="ka<?=$i?>" id="paynow">
							<option value="6">0-6</option>
							<option value="12">7-12</option>
							<option value="18">13-17</option>
						  </select>
						</div>
					</div>
				</div>
	...
	<div class="row">
		<div class="col-md-5"><p><strong>Choose Payment</strong><br/>
    ...
    <div class="row">
      ...
      <div class="col-md-3">
        <div class="form-group">
		  ...
		  <input type="hidden" name="kids" value="<?=$_SESSION['kids']?>">
          ...
          I Agree to <a href="terms-conditions.php" target="_blank" alt="Terms and Conditions">RCL Terms and Conditions.</a> </div>
      </div>
    </div>
