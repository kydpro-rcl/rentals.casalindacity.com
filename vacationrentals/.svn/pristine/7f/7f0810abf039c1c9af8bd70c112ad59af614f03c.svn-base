...
function mostrar_calendario2($mes,$ano,$villa_id){
  /* # //============================================================================================================================================  */
    $fecha_primer_dia=$ano."-".$mes."-01";
	...
	echo "<table class=\"table property-calendar-table\"><tr style=\"background-color:#e6e6e6;\">";
	echo "<td colspan=7 class=\"property-calendar-td property-calendar-td-month\" align=\"center\">$nombre_mes $ano</td>";
	echo "</tr>";
	echo '	<tr>
   ...
   /*#//=====================================================================================================================
	...
	for ($i=0;$i<7;$i++){
		if ($i < $numero_dia){
			/*#//si el dia de la semana i es menor que el numero del primer dia de la semana no pongo nada en la celda */
			echo '<td bgcolor="#F5F5F5" class="property-calendar-td">&nbsp;</td>';
		} else {
			/*#//-----------------------------------------------------------------------------------------------   */
			$esta_fecha=$ano."-".$mes."-".$dia_actual;
            $fecha_formato=date('Y-m-d',strtotime($esta_fecha));
            ...
            if (in_array($fecha_formato,$fechas_villa_ocupada)){
				$status=status_ocupado($villa_id, $fecha_formato);
				 switch($status){
					case 'long': $pintar_con="long"; $ocupaid_format='style="background-color:#FFCC66;"'; break;
					case 'short': $pintar_con="short"; $ocupaid_format='style="background-color:#FFCC66;"'; break;
					case 'maint': $pintar_con="maint"; $ocupaid_format='style="background-color:#FFCC66;"'; break;
					case 'owner': $pintar_con="owner"; $ocupaid_format='style="background-color:#FFCC66;"'; break;
				 }
			}else{
				//$pintar_con="disponible";
				$ocupaid_format='style="background-color:#ffffff;"';
			}
            /*}*/
            /*#//---------------------------------------------------------------------------------------------- */
			echo "<td $ocupaid_format class=\"property-calendar-td\">
					<span style=\"color:#000000;\">$dia_actual</span></td>";
			$dia_actual++;
		}
	}
	echo "</tr>";
	...
	while ($dia_actual <= $ultimo_dia){
        ...
        if (in_array($fecha_formato2,$fechas_villa_ocupada)){
		 ...
		 $status=status_ocupado($villa_id, $fecha_formato2);
		 switch($status){
					case 'long': $pintar_con2="long"; $ocupaid_format='style="background-color:#FFCC66;"'; break;
					case 'short': $pintar_con2="short"; $ocupaid_format='style="background-color:#FFCC66;"'; break;
					case 'maint': $pintar_con2="maint"; $ocupaid_format='style="background-color:#FFCC66;"'; break;
					case 'owner': $pintar_con2="owner"; $ocupaid_format='style="background-color:#FFCC66;"';  break;
				 }
		}else{
			//$pintar_con2="disponible";
			$ocupaid_format='style="background-color:#ffffff;"'; 
		}
        /*}
        #//-----------------------------------------------------------------------------------------------------------------------*/
		echo "<td " . dame_estilo($dia_actual) . " id=\"$pintar_con2\" $ocupaid_format ><span>$dia_actual</span></td>";
		$dia_actual++;
		$numero_dia++;
		/*#//si es el uñtimo de la semana, me pongo al principio de la semana y escribo el </tr>*/
		if ($numero_dia == 7){
