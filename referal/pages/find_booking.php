			...
			$reference=str_pad($reference, 9, "0", STR_PAD_LEFT);
		      if (isLength($reference,6,10)==true){ //verify an lenght between 6 and 10
				    ...
				    if ($result){
    					...
    					<? foreach ($result as $b){?>
						    <tr  bgcolor="#CCCCCC" onclick="reserva('../booking/reserva_details_RAP.php?id=<?=$b['busyid']?>','Details for Selection', 530, 800)" title="Click to see Reservation" onmouseover="this.style.backgroundColor='#87a2fa'; this.style.cursor='hand';this.style.cursor='pointer';" onmouseout="this.style.backgroundColor=''" >
