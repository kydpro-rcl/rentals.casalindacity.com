<? if($_POST){	   quitar_evento();/*quita la session de evento en caso de que existiera antes*/
	...
	//-----------------------END HIGHT AND LOW SEASONS QUANTITY-----------------------------------------------------------------------------
			...
			if ($night_qty>1){
              ...
              if ($night_qty<$price_settings['Long_nights']){ /*no hacer reservas online long term*/
					...
					$busy=$db->see_occupancy_online_2($fecha_de_inicio, $fecha_de_termino, $beds);//try
		             ...
		             if (!empty($villas_disponibles)){
				          ...
				          foreach ($villas_disponibles AS $d){
				           ...
				           <form method="post" name="villa_disponible" action="select_excursions.php">
                                <?
					            ...
					            <div class="villas_detalles"  ><!-- onmouseout="this.style.backgroundColor=''" onmouseover="this.style.backgroundColor='#00f4fd'" -->
	                                   <div class="detalles1">
								            ...
								            <h1 style="margin:0; padding:0; margin-bottom:5px;"><b>Villa No.<?=$d['no']?></b> - <?=$d['head']?> </h1>
								           	<?if ($LS_nights>0){?>
						                    	<p style="padding:0; margin:0; font-weight:bold;"> <?=$LS_nights?> nights at <?=number_format($d['p_low'],2);?> USD + <?=TAX_PERCENT?> TAX</p>
						                    <?}?>
						                    <?if ($HS_nights>0){?>
						                    	<p style="padding:0; margin:0; font-weight:bold;"><?=$HS_nights?> nights at <?=number_format($d['p_high'],2);?> USD + <?=TAX_PERCENT?> TAX</p>
					                        <?}?>
					                       ...
					                       ?>
	                                        ...
	                                        <!--// <p style="padding:0; margin:0;"><?=$HS_nights?> nights at <?=number_format($d['p_high'],2);?> USD + 16% TAX</p>//-->
					       		...
					       		<?}?>
					                   ...
					                   <div class="detalles2">
								          ...
								          <? if(($nights_qty>=30)&&($d['able_s']==1)){/*show as occupied if more than 29 nights and not monthly allow*/?>
								          	<img src="images/occupied.png" alt="Occupeid" />
								          <?}else{?>
								          	<input type="image" name="choose" src="images/select.png" >
								          <?}?>
								          </p>
	                                   </div>
					          </div>
	                  ...
	                  foreach ($villas_nodisponibles AS $vb){
                          ...
                          $vb['p_high']=special_event(date('Y-m-d',strtotime($fecha_de_inicio)), date('Y-m-d',strtotime($fecha_de_termino)), $vb['p_high']);
                           ...
                           //-----------------------apply weekly and monthly rate-------------------------------------------------------------
						             ...
						             $vb['p_low']=price_rent_online($nights_qty, $normal_price=$vb['p_low']);
						             $vb['p_high']=price_rent_online($nights_qty, $normal_price=$vb['p_high']);
						   //----------------------end apply weekly and monthly rate----------------------------------------------------
				        ...
				        ?>
					          ...
					          <div class="villas_detalles1"  >
					           ...
					           <h1 style="margin:0; padding:0; margin-bottom:5px;"><b>Villa No.<?=$vb['no']?></b> - <?=$vb['head']?> </h1>
								           	<?if ($LS_nights>0){?>
						                    	<p style="padding:0; margin:0; font-weight:bold;"> <?=$LS_nights?> nights at <?=number_format($vb['p_low'],2);?> USD + <?=TAX_PERCENT?> TAX</p>
						                    <?}?>
						                    <?if ($HS_nights>0){?>
						                    	<p style="padding:0; margin:0; font-weight:bold;"><?=$HS_nights?> nights at <?=number_format($vb['p_high'],2);?> USD + <?=TAX_PERCENT?> TAX</p>
					                        <?}?>
					               <?
					         ...
					         <p class="total"><sup>$</sup><span><?=number_format($total_amount)?></span></p>
					         <p class="price_t" style="padding-top:3px;">(Including Tax)</p>
					         <p style="text-align:center;padding:0; margin:0; margin-top:5px;"> <img src="images/occupied.png" alt="Occupeid" /></p>
					         </div>
	          		   </div>
