...
//============================END ACCESS RESTRICTED======================================================================
 if ($_GET['property_id']){
	...
	$datapic = new Basededatos2();//class picture db/table
	...
	#$directorio='../../photos/'.$villa['id'].'/';
	/*echo "<pre>";
	print_r($villa);
	echo "</pre>";   */
	$main_pic_folder='../../photos/'.$villa['id'].'/';
	...
	echo '<Property property_id="'.$villa['id'].'" last_update="'.$villa['date'].'">';
			...
			echo '</Details>';
			echo '<Descriptions>';
				echo '<PropertyHeadline>'.htmlentities($villa['headline']).'</PropertyHeadline>';
			...
			/* if ($fotos){
			  ...
			  $images = $datapic->getRows($villa['id']);
			if(!empty($images)){
				foreach($images as $row){
					echo '<Photo order="'.$row['img_order'].'">';
					echo '<URL>'.$main_pic_folder.rawurlencode($row['img_name']).'</URL>';
					//echo '<Description></Description>';
					echo '</Photo>';
				}
			}
