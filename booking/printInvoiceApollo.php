...
if ($_SESSION['info']){
 ...
 if ($_POST['ref']!=''){
	...
	$pdf->Cell(80);
	$pdf->SetFont('','B');
	$pdf->Cell(80,10,'INVOICE APOLLO');      //invoice title
    ...
    //--------------------------------------------------------------------------------------
     ...
     //--------------------------------------------------------------------------------------
     /*$pdf->Cell(40,6,'USD '.number_format($amount_nights,2),'R',0,'R',0);//total per rent
     $pdf->SetFont('');
     $pdf->Ln(4);*/
     //----------discount----------------------------------------------------------
    ...
    $pdf->SetFont('');
	 ...
	 $pdf->Ln();
	  $pdf->Cell(0,14,'INFORMATION FOR ACCOUNTING: ',0,'R',0);
   /* $pdf->SetFont('','U');
    $pdf->Cell(150,14,$client['address'],0,0);
    $pdf->SetFont('');*/
	$margin1=130; $margin2=15; $margin3=30; $margin4=0;
	$TotalNoches=$o['NLS']+$o['NHS'];
	$pdf->Ln(5);
	$pdf->Cell($margin1,14,'Accounting fee: ',0,'R',0);
	$AccFee=number_format(((13.85/7)*$TotalNoches),2);
	$pdf->Cell($margin2,14,$AccFee,0,'R',0);
	$pdf->Cell($margin3,14,'Adm fee: ',0,'R',0);
	$AdmFee=number_format(((5.77/7)*$TotalNoches),2);
	$pdf->Cell(0,14,$AdmFee,0,'R',0);
	
	$pdf->Ln(5); 
	$pdf->Cell($margin1,14,'Internet: ',0,'R',0);
	$IntFee=number_format(((6.92/7)*$TotalNoches),2);
	$pdf->Cell($margin2,14,$IntFee,0,'R',0);
	$pdf->Cell($margin3,14,'Gas: ',0,'R',0);
	$GasFee=number_format(((5.77/7)*$TotalNoches),2);
	$pdf->Cell(0,14,$GasFee,0,'R',0);
	
	$pdf->Ln(5); 
	$pdf->Cell($margin1,14,'Water: ',0,'R',0);
	$WatFee=number_format(((5.77/7)*$TotalNoches),2);
	$pdf->Cell($margin2,14,$WatFee,0,'R',0);
	$pdf->Cell($margin3,14,'Pool/garden: ',0,'R',0);
	$poolService=(($villa[0]['p_out_clear']*12)/52);
	$PooFee=number_format((($poolService/7)*$TotalNoches),2);
	$pdf->Cell(0,14,$PooFee,0,'R',0);
	
	$pdf->Ln(5); 
	$pdf->Cell($margin1,14,'Maid: ',0,'R',0);
	$maidService=(($villa[0]['p_in_clear']*12)/52); 
	$MaidFee=number_format((($maidService/7)*$TotalNoches),2);
	$pdf->Cell($margin2,14,$MaidFee,0,'R',0);
	$pdf->Cell($margin3,14,'Maintenance: ',0,'R',0);
	$MaiFee=number_format(((66.92/7)*$TotalNoches),2);
	$pdf->Cell(0,14,$MaiFee,0,'R',0);
	$pdf->Ln(5); 
	
	$pdf->Cell($margin1,14,'Electricity: ',0,'R',0);
	if($villa[0]['bed']==2){ $EleFee=number_format(((159.23/7)*$TotalNoches),2);}else{$EleFee=number_format(((182.30/7)*$TotalNoches),2);}
	$pdf->Cell($margin2,14,$EleFee,0,'R',0);
	/*$pdf->Cell(70,14,'Adm fee: ',0,0);
	$pdf->Cell(90,14,'X',0,0);*/
	$amtSF=$AccFee+$AdmFee+$IntFee+$GasFee+$WatFee+$PooFee+$MaidFee+$MaiFee+$EleFee;
	$margin11=165; $margin12=0;
	$pdf->Ln(5); 
	 $pdf->SetFont('','U');
	$pdf->Cell($margin11,14,'TOTAL SERVICES AND FEES: ',0,'R',0);
	$pdf->Cell(0,14,$amtSF,0,'R',0);
	$pdf->SetFont('');
	$pdf->Ln(5); 
	$amtVilla=$amount_nights-$amtSF; 
	$pdf->Cell($margin11,14,'VILLA AMOUNT: ',0,'R',00);
	$pdf->Cell(0,14,$amtVilla,0,'R',0);
	$pdf->Ln(5); 
	$pdf->Cell($margin11,14,'15% commission RCL: ',0,'R',0);
	$amtCom=$amtVilla*0.15;
	$pdf->Cell(0,14,$amtCom,0,'R',0);
	$pdf->Ln(5); 
	
	$pdf->Cell($margin11,14,'Amount left to owner: ',0,'R',0);
	$amtOwe=$amtVilla-$amtCom;
	$pdf->Cell(0,14,$amtOwe,0,'R',0);
	$pdf->Ln(5); 
