...
$loc=$db->showTable_all($table='villas_location', $order='id');
/*echo "<pre>";
print_r($loc);
echo "</pre>";*/
$location_gral="";
$total_location=count($loc);
$i=1;
foreach($loc AS $l){
	
	if($total_location!=$i){
		$location_gral.="['".$l['locationName']."', ".$l['latitude'].", ".$l['logitude'].", ".$i."],";
	}else{
		$location_gral.="['".$l['locationName']."', ".$l['latitude'].", ".$l['logitude'].", ".$i."]";
	}
	$i++;
...
<html>
  <head>
    ...
    <meta charset="utf-8">
    <title>All Location for Residencial Casa Linda</title>
    <style>
