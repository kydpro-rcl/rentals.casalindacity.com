...
if ($_SESSION['info']){
 ...
 if ($_POST['ref']!=''){
    ...
    if (($o['status']==0)||($o['status']==5)||($o['status']==7)||($o['status']==1)||($o['status']==2)||($o['status']==3)||($o['status']==4)||($o['status']==6)||($o['status']==12)||($o['status']==13)||($o['status']==14)||($o['status']==7)||($o['status']==19)||($o['status']==20)||($o['status']==21)||($o['status']==8)||($o['status']==9)||($o['status']==10)||($o['status']==11)||($o['status']==15)||($o['status']==16)||($o['status']==17)||($o['status']==18)||($o['status']==22)||($o['status']==23)||($o['status']==24)||($o['status']==25)){    	header('Location:invoices_buyer.php?error=We sorry, this is not a buyer rental');
    	die();
    }
	...
	$pdf->Cell(80);
	$pdf->SetFont('','B');
	$pdf->Cell(70,10,'INVOICE');      /*invoice title*/
    ...
    $pdf->Cell(45,14,$fecha_de_pago_actual.' / '.$fecha_de_pago_proxima,0,0);
     ...
     $encabezado=array('Quantity','Details','Price','Total');
      ...
      if (($o['EN']>0)&&($pagos_totales==$pago_actual)){      	if($pagos_totales==1){ /*verify if this booking has less than 30 nights */         $total_a_cobrar=($o['EN']*($o['PL']/30));
         $data=array( 1=>array($o['EN'],'Nights ',number_format(($o['PL']/30),2),number_format($total_a_cobrar,2)));
      	}else{      	$total_a_cobrar=($o['EN']*($o['PL']/30));
      	$data=array( 1=>array($o['EN'],'Extra Nights',number_format(($o['PL']/30),2),number_format($total_a_cobrar,2)));
      	}
      }else{      	$total_a_cobrar=(1*$o['PL']);        $data=array( 1=>array('1','Monthly payment per villa',number_format($o['PL'],2),number_format($total_a_cobrar,2)));      }

     $pdf->FacturaTable($encabezado,$data);
     $pdf->Ln();
     $pdf->SetFont('','B');
     $pdf->Cell(145,6,'Amount for villa = ','L',0,'R',0);
     ...
     $pdf->Ln();
     $pdf->SetFont('','B');
     $pdf->Cell(145,6,'TOTAL WITH SERVICES = ','L',0,'R',0);
     ...
     $pdf->Cell(185,0,'','T');
         ...
         $pdf->Ln();
         $pdf->Cell(30,6,'Client Responsability',0,0);
         $pdf->Ln();
         $pdf->SetFont('Times','',8);
    ...
    $pdf->Cell(80,6,' Monthly payments to USD '.number_format($o['PMV'],2).' Each.',0,0);

	if($o['EN']>0){
      $pdf->Cell(80,6,' Extra nights to USD '.number_format($o['ppn'],2).' Each.',0,0);
    }
    ...
    /*//signature end*/
    /*============================================PAGE NUMBER 2 BELOW FOR SERVICES ====================================================================*/
      $pdf->AddPage();
    $pdf->SetAuthor('ing.joseluis@msn.com');
	$pdf->SetTitle('Residencial Casa Linda-Invoice');
	$pdf->SetFont('Times','',16);
	...
	$pdf->Cell(80);
	$pdf->SetFont('','B');
	$pdf->Cell(80,10,'INVOICE');      /*//invoice title*/
	$pdf->SetFont('Times','',12);
    ...
    $pdf->Cell(45,14,$fecha_de_pago_actual.' / '.$fecha_de_pago_proxima,0,0);
     ...
     $encabezado=array('Quantity','Details','Price','Total');
      ...
      if (($o['EN']>0)&&($pagos_totales==$pago_actual)){         $info_services=array();
         ...
         foreach ($servicios_reserva_long as $sl){
                    ...
                    $pdf->Ln(5); */
					/*$total_services_long+=$sl['price'];*/
			$total_a_cobrar=$o['EN']*($sl['price']/30);
			$total_a_servicios+=$total_a_cobrar;
			$services_booking=array($o['EN'],ucfirst($sl['name']),number_format(($sl['price']/30),2),number_format($total_a_cobrar,2));
			array_push($info_services,$services_booking);
		 }

      	if($pagos_totales==1){ /*verify if this booking has less than 30 nights and charge the whole month*/
         ...
         foreach ($servicios_reserva_long as $sl){
                    ...
                    $pdf->Ln(5); */
					/*$total_services_long+=$sl['price'];*/
			$total_a_cobrar=1*($sl['price']);
			$total_a_servicios+=$total_a_cobrar;
			$services_booking=array('1',ucfirst($sl['name']),number_format(($sl['price']),2),number_format($total_a_cobrar,2));
			array_push($info_services,$services_booking);
		 }
		  $data=$info_services;
     ...
     $pdf->Ln();
     /*$pdf->SetFont('','B');
     $pdf->Cell(145,6,'Amount for services = ','L',0,'R',0);
     ...
     $pdf->Cell(40,6,'USD '.number_format($total_a_servicios,2),'R',0,'R',0);
     $pdf->SetFont('');
     $pdf->Ln();
     $pdf->Cell(185,0,'','T');
         //services
