<?
 class subDB extends DB{
   ...
   public function authenticateUser($user,$pass) {
	...
	public function insert_short_reserva($ref, $id_ocupacion, $id_customer, $adults_qty, $children_qty, $interm_id, $qty_nights, $HS_nights, $LS_nights, $price_per_night, $priceHS, $amount_commision, $sub_total_rent, $ITBIS, $services_amount, $deposit, $general_amount, $status){


    $this->insert_reserve($ref, $id_ocupacion, $id_customer, $adults_qty, $children_qty, $interm_id, $qty_nights, $HS_nights, $LS_nights, $price_per_night, $priceHS, $amount_commision, $sub_total_rent, $ITBIS, $services_amount, $deposit, $general_amount, $status);
	return $this->getInsertId();
	}
   ...
   //-------------------------------end long term info ------------------------
	...
	public function new_user($username, $password, $level, $name, $lastname, $email, $phone, $info, $active){

	#$date=date("Y-m-d h:m:s");
	$date=date("Y-m-d G:i:s");
	$result=$this->Insert_User($username, $password, $level, $name, $lastname, $email, $phone, $info, $active, $date);
	 if (!$result){
 	 return false;
 	 }else{
	...
	public function newOwner($username, $password, $name, $lastname, $email, $phone, $movil, $fax, $cedula, $passport, $language, $zip, $address, $country, $photo, $rent_contract, $serv_contract, $comentario, $active, $id_adm){

	$date=date("Y-m-d G:i:s");
	$result=$this->insertOwner($username, $password, $name, $lastname, $email, $phone, $movil, $fax, $cedula, $passport, $language, $zip, $address, $country, $photo, $rent_contract, $serv_contract,$comentario, $date, $active, $id_adm);

 	 return true;

	}
	public function UpdateOwner($id, $username, $password, $name, $lastname, $email, $phone, $movil, $fax, $cedula, $passport, $language, $zip, $address, $country, $photo, $rent_contract, $serv_contract, $comentario, $active, $id_adm){

	$date=date("Y-m-d G:i:s");
	$result=$this->updOwner($id, $username, $password, $name, $lastname, $email, $phone, $movil, $fax, $cedula, $passport, $language, $zip, $address, $country, $photo, $rent_contract, $serv_contract, $comentario, $date, $active, $id_adm);
   ...
   public function UpdateUser($id, $level, $name, $lastname, $email, $phone, $info, $active){

		$date=date("Y-m-d G:i:s");
		$result=$this->updUsers($id, $level, $name, $lastname, $email, $phone, $info, $active, $date);
...
//--------------ASIGN BOOKING TO REFERAL--------------------------------------------------------------------------------------------------------------------------
   public function Assign_a_booking($reference, $referal, $id_adm){
	$date=date("Y-m-d G:i:s");
    $id_update="";
    $result=$this->assignBooking($reference, $referal, $id_adm, $date, $id_update);
	//return $this->getInsertId();
	return $result;
	}
...
//-----------------INSERT ASSIGN MODIFIED --------------------------------------------------------------------------------------------------------------------
	...
	//--------------insert promotion--------------------------------------------------------------------------------------------------------------------------
   public function pro_in($code, $tipo, $monto_porc, $mdays, $ddays,  $desde, $hasta, $bookingfrom, $bookingto, $id_adm, $activo){
	$date=date("Y-m-d G:i:s");
    $result=$this->insert_promotion($code, $tipo, $monto_porc, $mdays, $ddays,  $desde, $hasta, $bookingfrom, $bookingto, $id_adm, $activo, $date);
	return $result;
	}
//-----------------update promotion------------------------------------------------------------------------------------------------------------------
	public function pro_up($id, $code, $tipo, $monto_porc, $mdays, $ddays,  $desde, $hasta, $bookingfrom, $bookingto, $id_adm, $activo){
  	 $fecha=date("Y-m-d G:i:s");
	 $result=$this->update_promotion($id, $code, $tipo, $monto_porc, $mdays, $ddays, $desde, $hasta, $bookingfrom, $bookingto, $id_adm, $activo, $fecha);
	return $result;
	}

	//-------------- INSERT DISCOUNT TO BOOKING ----------------------------------------------------------------------------------------------------------------------
   public function insert_discount($ref,$pro_code,$pro_id,$pro_from,$pro_to,$pro_type,$pro_qty,$min_days,$qty_days, $id_adm){
	$fecha=date("Y-m-d G:i:s");
    $update="";
    $result=$this->insert_discount_DB($fecha,$ref,$pro_code,$pro_id,$pro_from,$pro_to,$pro_type,$pro_qty,$min_days,$qty_days, $id_adm,$update);
	return $result;
	}
//-----------------UPDATE DISCOUNT TO BOOKING ---------------------------------------------------------------------------------------------------------------
	public function update_discount($id,$ref,$pro_code,$pro_id,$pro_from,$pro_to,$pro_type,$pro_qty,$min_days,$qty_days, $id_adm,$update){
	$fecha=date("Y-m-d G:i:s");
	$result=$this->update_discount_DB($id,$fecha,$ref,$pro_code,$pro_id,$pro_from,$pro_to,$pro_type,$pro_qty,$min_days,$qty_days, $id_adm,$update);
	return $result;
	}

//-----------------INSERT A DISCOUNT MODIFIED --------------------------------------------------------------------------------------------------------------------
	public function insert_discount_modified($fecha,$ref,$pro_code,$pro_id,$pro_from,$pro_to,$pro_type,$pro_qty,$min_days,$qty_days, $id_adm){
		$this->insert_discount_modified_DB($fecha,$ref,$pro_code,$pro_id,$pro_from,$pro_to,$pro_type,$pro_qty,$min_days,$qty_days, $id_adm);
	return $this->getInsertId();
	}
