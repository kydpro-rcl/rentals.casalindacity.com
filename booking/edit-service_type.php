...
if ($_SESSION['info']){
	if ($_SESSION['info']['level']==1){
	    ...
	    if ($_POST){
	        ...
	        $photo=$_POST['old_pic']; //empty path
	        }

	         if (!$_GET['error']){
			    $db=new subDB();
			    $insert_service=$db->upd_type_serv($_POST['id'],$t=$_POST['type'], $p=$photo, $m=$_POST['msg'], $nl=$_POST['nl'], $l=$_POST['url']);
			     if ($insert_service){
	               	$_GET['p']='a';
