...
if($_SESSION['COUNT']['QTY']==''){/*only query all the fist time*/
		//$bookings_found=$data->bookings_referral_ready_pickup();
		/*$bookings_found=$data->bookings_referral_status($referralid=$_SESSION['REFERAL']['SHOWING'], $status=1);
...
if($_POST['referral_showing']!=''){
	...
	$bookings_found=$data->bookings_referral_status_ft($referralid=$_SESSION['REFERAL']['SHOWING'], $status=1, $from=$start_date, $to=$end_date);
	$total_records=$data->getAffectedRows();
	$_SESSION['COUNT']['QTY']=$total_records;
 ...
 if(!$_POST['book_refs']){
	...
	# $bookings_found=$data->referral_commission_state($id_referral=$_SESSION['REFERAL']['SHOWING'], $from_records=$paging_info['si'], $to_records=$items_pp, $status_comission=1);
	 //$bookings_found=$data->bookings_referral_unpaid();	
		 #$bookings_found1=$data->referral_commission_state1($id_referral=$_SESSION['REFERAL']['SHOWING'], $from_records=$paging_info['si'], $to_records=$items_pp, $status_comission=1);
		if($bookings_found1){
      ...
      <form method="post" action="ready_to_pickup.php">
		<p style="font-size:10px; padding-left:15px; color:blue;"><strong>Total comisiones sin cobrar: <?=$count?></strong>
			
			<!--<span style="margin-left:35px;">Referral:
			<select name="referral_showing" onchange="window.location='ready_to_pickup.php?rs='+this.value">
				<option value="0">See All</option>
				<?php
				/*foreach($referrals AS $k){
				?>
	 ...
	 #numeros_de_paginas($paging_info,$max=7,'ready_to_pickup.php');
