...
//require_once('inc/session.php');
  $id=$_GET['i'];
		/*if ($_SESSION['info']){*/
		 if(!$id){die('Error: missing id on booking details');}
		  ...
		  $montoTotalPagado=$db->amountRef($ocupabilidad['ref'],'1');/*paid*/
		  ...
		  $montoTotalSDevuelto=$db->amountRef($ocupabilidad['ref'],'4');/*security refund*/
		   $montoTotalSTransferido=$db->amountRef($ocupabilidad['ref'],'5');/*Move to another booking*/
			...
			<h2 style="text-align:center">Applying payment transactions to booking No.:<span style="color:green;"> <?=$ocupabilidad['ref'];?></span></h2><hr>		
	  <?
	 if ($_GET['infoNote']){?>
			 	<div style="text-align:center; color:#080563; background-color:yellow; padding:5px;"><strong>Notice: <?=$_GET['infoNote'];?></strong></div>
	 <?}?>
	 <?
	 if ($_GET['infoNote1']){?>
			 	<div style="text-align:center; color:#FFFFFF; background-color:green; padding:5px;"><strong>Notice: <?=$_GET['infoNote1'];?></strong></div>
	 <?}?>
	 <form method="post" action="applyPayment.php">
	 <table align="center"><tr><td>
	<fieldset><legend>Transactions</legend>
	 <p style="text-align:right; margin-right:10px;">Transaction amount<input type="text" name="pamount" value="" />
	 <? $tipoPagos=paymentsType();?>
	 <select name="ptype">
	 <? foreach($tipoPagos AS $k=>$v){?>
		<option value="<?=$k?>"><?=$v?></option>
	 <?}?>
	 </select>
	 
	 <? $classP=paymentsClass();?>
	 <select name="pclass">
	 <? foreach($classP AS $k=>$v){?>
		<option value="<?=$k?>"><?=$v?></option>
	 <?}?>
	 </select>
	 </p>
	 
	 <p style="text-align:right; margin-right:10px;">Transaction ID/Ref nr:<input type="text" name="refID" value=""/></p>
	 <p style="text-align:right; margin-right:10px;">Notes:<textarea name="pnote" cols="35" rows="5"></textarea></p>
	<input type="hidden" name="id_adm" value="<?=$_SESSION['info']['id'];?>" />
	<input type="hidden" name="i" value="<?=$id;?>" />
	<input type="hidden" name="ref" value="<?=$ocupabilidad['ref'];?>" />
	  <!--<p style="text-align:right; font-size:10px; color: #06F; font-family:Arial, Helvetica, sans-serif; ">-->
...
<table id="web-buttons-idt4wi7" width="100%" cellpadding=0 cellspacing=0 border=0><tr>
<td style="padding-right:0px" title ="Print Invoice">
<a href="invoices.php?ref=<?=$ocupabilidad['ref'];?>"  target="_blank" title="Print Invoice" style="background-image:url(images/btt4wi7.png);width:73px;height:26px;display:block;"><br/><!--<input type="image" src="images/Print Invoice_0.png.png" alt="Submit">--></a></td>
<td align="right"><!--<input class="book_but" type="submit" name="confirm" value="Apply Payment"  title="click to apply payment" />--><input type="image" src="images/ApplyPayment_0.png" alt="Submit"></td></tr></table>
	  <!--</p>-->
	  </td></tr></table>
	  </fieldset>
	</form>
		<div style="text-align:center; color:blue; background-color:#f5f5f5; padding:5px;text-transform:uppercase; border:0px solid #C8E775; width:75%;box-shadow:#888888 0 0 1px 1px;margin:0 auto;"><strong>
		<? if($_GET['sh']=='y'){$verdet='n';}else{$verdet='y';}?>
		
		<a href="applyPayment.php?i=<?=$_GET['i']?>&sh=<?=$verdet?>">Click to see or hide the details for this booking</a>
...
<b>Reference:</b> <a href="edit-booking.php?refnumb=<?=$ocupabilidad['ref'];?>" title="Change this booking" target="_blank"><u><?=$ocupabilidad['ref'];?></u></a> <b>Status:</b>
			  <?
		       ...
		       if ($ocupabilidad['dep']>0){/*(($ocupabilidad['status']==2)||($ocupabilidad['status']==6))*/ echo " <strong>Deposited:</strong> <span class='azul2'>".$ocupabilidad['dep']." USD</span>";}
					...
					/* echo '<pre>';
					 ...
					 <p><?php /*=$total_services*/?></p>
						...
						// print_r($agent_comision[0]);
						 ...
						 $monto_por_villa=(($LS_monto+$HS_monto)-$descuento-$this_disc[0]['discounted']);
		   ...
		   <?/* if (!empty($servicios_reserva)){
			...
			<? /*if ($ocupabilidad['adm']!="0"){*/?>
				...
				<? if ($ocupabilidad['upd']>0){?>
					...
					$link= new DB(); $made=$link->getUserDetails($fecha_modificado[0]['id_adm']);
					?>
