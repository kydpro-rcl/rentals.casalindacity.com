			...
			if ($_GET['refnumb']) $reference=$_GET['refnumb'];
			 ...
			 if (($reference!="")&&($f_date!="")&&($t_date!="")&&($villa_id!="")&&($status!="")&&($client!="")){
			   ...
			   if(!$cant_edit>0){
					   ...
					   //echo $villa_selected[0]['no']; echo "<br/>";
					  // echo $f_date; echo "<br/>";
					   $p=$db->get_season3_prices($startdate=$f_date, $pricelow=$villa_selected[0]['p_low'], $priceshoulder=$villa_selected[0]['p_shoulder'], $pricehigh=$villa_selected[0]['p_high']);	
					   //print_r($p);
					   //die();
					   if($p['price']==0){
	...
	/*//-----------------------HIGHT AND LOW SEASONS QUANTITY-----------------------------------------------------------------------------*/
								 ...
								 if (is_date($fecha_empiezas)){
									...
									"1")){   /* //solo HS*/
						             ?>
						            	<? if ($_SESSION['info']['level']==1){?>
								       		Price&nbsp;US$&nbsp;<span class="azul"><input type="text" name="priceHS" size="5" value="<?=number_format($price_HS,2);?>" /></span>
								       		<input type="hidden" name="price" value="0" />
								       		<input type="hidden" name="HS_nights" value="<?=$HS_nights;?>" />
							            	<input type="hidden" name="LS_nights" value="<?=$LS_nights;?>" />
							            <?}else{?>
							            	Price2&nbsp;US$&nbsp;<span class="azul"><?=number_format($price_HS,2);?></span>
						            ...
						            <?
						             ...
						             ?>
							             <? if ($_SESSION['info']['level']==1){?>
								       		Price&nbsp;US$&nbsp;<span class="azul"><input type="text" name="price" size="5" value="<?=number_format($price_LS,2);?>" /></span>
								       		<input type="hidden" name="priceHS" value="0" />
								       		<input type="hidden" name="HS_nights" value="<?=$HS_nights;?>" />
							            	<input type="hidden" name="LS_nights" value="<?=$LS_nights;?>" />
							            <?}else{?>
							            	Price2&nbsp;US$&nbsp;<span class="azul"><?=number_format($price_LS,2);?></span>
						            ...
						            <?
						             	...
						             	<? if ($_SESSION['info']['level']==1){?>
								       		Price&nbsp;US$&nbsp;<span class="azul"><input type="text" name="price" size="5" value="<?=number_format($price_LS,2);?>" /></span>
								       		Price2&nbsp;US$&nbsp;<span class="azul"><input type="text" name="priceHS" size="5" value="<?=number_format($price_HS,2);?>" /></span>
								       		<input type="hidden" name="HS_nights" value="<?=$HS_nights;?>" />
							            	<input type="hidden" name="LS_nights" value="<?=$LS_nights;?>" />
							            <?}else{?>
							            	Price&nbsp;US$&nbsp;<span class="azul"><?=number_format($price_LS,2);?></span>
							            	<input type="hidden" name="price" value="<?=number_format($price_LS,2);?>" />
							            	Price2&nbsp;US$&nbsp;<span class="azul"><?=number_format($price_HS,2);?></span>
							            	<input type="hidden" name="priceHS" value="<?=number_format($price_HS,2);?>" />
							            	<input type="hidden" name="HS_nights" value="<?=$HS_nights;?>" />
