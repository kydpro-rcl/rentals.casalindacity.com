...
<p class="header">RESERVACIONES SIN PAGAR</p>
<?php
$data= new getQueries ();
...
<?php
$data= new getQueries ();
if($_SESSION['COUNT']['QTY1']==''){/*only query all the fist time*/
		//$bookings_found=$data->bookings_referral_unpaid();
		/*$bookings_found=$data->bookings_referral_status($referralid=$_SESSION['REFERAL']['SHOWING'], $status=0);
		$total_records=$data->getAffectedRows();
		$_SESSION['COUNT']['QTY1']=$total_records;*/
}
	 
/*	 
if($_GET['rs']!=''){
	$_SESSION['REFERAL']['SHOWING']=$_GET['rs'];
	$bookings_found=$data->bookings_referral_status($referralid=$_SESSION['REFERAL']['SHOWING'], $status=0);
	$total_records=$data->getAffectedRows();
	$_SESSION['COUNT']['QTY1']=$total_records;
}*/
...
if($_POST['referral_showing']!=''){
	...
	$bookings_found=$data->bookings_referral_status_ft($referralid=$_SESSION['REFERAL']['SHOWING'], $status=0, $from=$start_date, $to=$end_date);
	$total_records=$data->getAffectedRows();
	$_SESSION['COUNT']['QTY']=$total_records;
 ...
 if(!$_POST['book_refs']){
	 ...
	 #$bookings_found=$data->referral_commission_state($id_referral=$_SESSION['REFERAL']['SHOWING'], $from_records=$paging_info['si'], $to_records=$items_pp, $status_comission=0);
	 #$bookings_found1=$data->referral_commission_state1($id_referral=$_SESSION['REFERAL']['SHOWING'], $from_records=$paging_info['si'], $to_records=$items_pp, $status_comission=0);
	if($bookings_found1){
	...
	echo "</pre>";*/
     ...
     if ($bookings_found){		
      ...
      <form method="post" action="unpaid.php">
		<p style="font-size:10px; padding-left:15px; color:blue;"><strong>Total comisiones sin pagar: <?=$count?></strong>
			<span style="margin-left:35px;">Accion con los seleccionados:
			<select name="accion">
				<option value="1">Listo para recoger</option>
				<option value="2">Pagado</option>
			</select>
			</span>
			
			<!--<span style="margin-left:35px;">Referral:
			<select name="referral_showing" onchange="window.location='unpaid.php?rs='+this.value">
				<option value="0">See All</option>
				<?php
				/*foreach($referrals AS $k){
				?>
   ...
   <!--//   codigo para promotion code//-->
       ...
       <p style="margin-left:35px;">  <input class="book_but" type="submit" name="imprimir" value="Ir"/>

       </p>
       </form>
	  <?
	  
	   #numeros_de_paginas($paging_info,$max=7,'unpaid.php');
 ...
 $cantid=0;
 $link=new DB();
   if ($_POST['accion']==1){//pasar a recoger
      foreach($_POST['book_refs'] as $k=>$v){
      $cantid++;
      	$link->pickup_check_referral($v);
      }
	$acciones=" lista para recoger en la oficina";
   }elseif($_POST['accion']==2){ //esta pagado
	 foreach($_POST['book_refs'] as $k=>$v){
	      $cantid++;
