...
<hr />
<form name="new_villa" method="post"  action="new-client1.php" enctype="multipart/form-data">
...
<p id="fields">Phone2: <input class="input" type="text" name="phone2"  value="<?=$_POST['phone2']?>" /></p>
...
<option value="0" <? if ((!$_POST['intermediario'])&&(!$_SESSION['referal']['new'])) echo "selected='selected'"; ?>>None</option>
<? $int=new getQueries(); $inter=$int->show_all_active('commission','id');
foreach ($inter as $k){?>
	<option value="<?=$k['id']?>" <? if (($_POST['intermediario']==$k['id'])||($_SESSION['referal']['new']==$k['id'])) echo "selected='selected'"; ?>><? echo $k['name']." ".$k['lastname'];?></option>
<? }?>
<?/* if ($_SESSION['referal']['new']) unset($_SESSION['referal']['new']);*//*QUITAR REFERAL*/?>
</select><!--<br/><a href="javascript: void(0)" onclick="windowOpener('400','400','referal','new-interm.php');">New Referal</a>--></p>

<p id="fields">Sale Agent: <select class="input" name="sale" ><!--only for admin empty for others-->
<option value="0" <? if ((!$_POST['intermediario'])&&(!$_SESSION['referal']['new'])) echo "selected='selected'"; ?>>None</option>
<? $int=new getQueries(); $inter=$int->show_all_active('commission','id');
foreach ($inter as $k){?>
	<option value="<?=$k['id']?>" <? if (($_POST['sale']==$k['id'])||($_SESSION['referal']['new']==$k['id'])) echo "selected='selected'"; ?>><? echo $k['name']." ".$k['lastname'];?></option>
<? }?>
<?/* if ($_SESSION['referal']['new']) unset($_SESSION['referal']['new']);*//*QUITAR REFERAL*/?>
</select><!--<br/><a href="javascript: void(0)" onclick="windowOpener('400','400','referal','new-interm.php');">New Referal</a>--></p>
