...
<form method="post" action="bookings_per_months_type.php" >
	<p id="fields" style="text-align:center;">Month:
	  ...
	  <select name="type">
		 <option value="1" <? if($_POST['type']==1){?> selected="selected" <?}?> >Online</option>
         <option value="0" <? if($_POST['type']==0){?> selected="selected" <?}?> >Internal</option>
		 <option value="2" <? if($_POST['type']==2){?> selected="selected" <?}?> >Referral</option>
 ...
 if($_POST){
     ...
     $dias_arreglos_mes=arreglos_days($start_date=$fecha_inicial_mes, $end_date=$fecha_final_mes);
       ...
       $db= new getQueries();
		$result=$db->booking_per_months_type($month=$_POST['m'], $year=$_POST['y'], $year=$_POST['type']);
		...
		if ($result){
			...
			<tr>
				    						...
				    						<tr bgcolor="#a6cdf4">
												...
												<td align="center" class="blue_dark" style="font-size:10px;"><strong>date</strong>
					    						</td>
				    						</tr>
			<? 
			$total_mes_villas='';
			  ...
			  if(($b['status']!=0)&&($b['status']!=5)&&($b['status']!=7)&&($b['status']!=19)&&($b['status']!=20)&&($b['status']!=21)&&($b['status']!=22)&&($b['status']!=23)&&($b['status']!=24)&&($b['status']!=25)){		/*only if it's not owners, cancelled, maintenance*/
				  ...
				  if(($b['status']==3)||($b['status']==10)||($b['status']==13)||($b['status']==17)||($b['status']==27)||($b['status']==31)||($b['status']==35)){		/*what is unferified*/
					...
					<td style="font-size:9px" ><? $villa=$db->villa($b['villa']); echo $villa[0]['no'];?></td>
						...
						<td style="font-size:11px; text-align:right;"  > <?=$b['date'];?></td>
					</tr>
				 <? 
