...
if ($_SESSION['info']){
 ...
 if ($_POST['referal_id']!=''){
	...
	//$_POST['ref']=str_pad($_POST['ref'], 9, "0", STR_PAD_LEFT);
  	...
  	$bookings_found=$db->booking_per_months_referral($month=$_POST['month'], $year=$_POST['year'], $id_referral=$_POST['referal_id']);;
	...
	//Pie de página
	    ...
	    $this->SetFont('','B');
	    //Cabecera
	    $w=array(20,30,30,30,15,35,35,25,25);
	     $this->Ln();
	...
	//$pdf->Cell(80);
	$pdf->SetFont('','B');
	$pdf->Cell(10,10,'REPORT OF BOOKINGS FOR EXPEDIA.COM');      //report title
	$pdf->SetFont('Times','',9);

	$pdf->Ln();


     $encabezado=array('BOOKING','STATUS','DATE','CLIENT','VILLA','FROM','TO','EXPEDIA ID','TOTAL USD');


      $contador=0;
      $bookings_found_referal=array();
      $expedia_payable=0;
      foreach($bookings_found as $book){      	 $link= new getQueries ();      	 $expedia=$link->show_any_data_limit1('expedia', 'rcl_ref', $book['ref'], '=');
		   ...
		   if($book['status']!=0){
      		...
      		//*******************************************************************************************************************	   if($book['status']!=0){
	     $este_booking=array($book['ref'],$status_result,$book['date'], $este_cliente,$numero_villa, $desde,$hasta, $exp['exp_id'], number_format($exp['exp_amount'],2));
    ...
    // $pdf->SetFont('');
     ...
     $pdf->Cell(185,0,'','T');
     $pdf->Ln();
     $pdf->Cell(220,10,'Total Payment =','',0,'R');   $pdf->Cell(25,10,number_format($expedia_payable,2),'',0,'R');
      /*$pdf->Ln(5);
	...
	header('Location:invoices.php?error=Expedia.com id empty');
 }
}else{
